...
class DashboardFragment : Fragment(), KodeinAware, ByFilterListener {
    ...
    override fun onCreateView(
        ...
        filterViewModel =
            ViewModelProvider(this, filterViewModelFactory).get(SmartFilterViewModel::class.java)
        ...
        binding.tvAllShared.setOnClickListener { v: View? ->
            Utility.movetoFragment(
                activity,
                SharedLocationFragment()
            )
        }
...
//                        return@setOnClickListener
                    ...
                    8 -> {
                        if (!loginMember?.role.isNullOrEmpty() && loginMember?.role != getString(R.string.USER)) {
                            ...
                            binding.llParent.snackbar(
                                getString(R.string.admin_only),
...
//                }
    ...
    inner class SharedProfileAdapter internal constructor(private val list: ArrayList<Member>) :
        RecyclerView.Adapter<SharedLocationViewHolder>() {
        override fun onCreateViewHolder(
            parent: ViewGroup,
            viewType: Int
        ): SharedLocationViewHolder {
            val view = LayoutInflater.from(parent.context)
                .inflate(R.layout.shared_list_profle, parent, false)
        ...
        override fun onBindViewHolder(holder: SharedLocationViewHolder, position: Int) {
            ...
            when {
                duplicateIds.contains(member.id) -> {
                    holder.cardViewRecentList.setCardBackgroundColor(
        ...
        private fun applyProfilePicture(holder: SharedLocationViewHolder, member: Member) {
            ...
            if (!TextUtils.isEmpty(member.profilePic)) {
                ...
                try {
                    Glide.with(activity!!).load(url).placeholder(R.drawable.user_face)
                        .apply(RequestOptions.circleCropTransform()).thumbnail(1f)
